...
export const updateWeather = function (lat, lon) {
  ...
  const ctx = document.getElementById('pastWeatherChart')?.getContext('2d');
  ...
  fetchData(url.currentWeather(lat, lon), function (currentWeather) {
    ...
    /*********** */
    fetchData(url.current(lat, lon), (data) => {
      if (!data) return;
      const currentWeather = data;
      const ctx = document.getElementById('WeatherSummary');
      if (!ctx) return;
      const chart = new Chart(ctx.getContext('2d'), {
        type: 'bar',
        data: {
          labels: ['Temperature', 'Humidity', 'Wind Speed', 'Clouds'],
          datasets: [{
            label: 'Today\'s Weather Summary',
            data: [
    ...
    fetchData(url.forecast(lat, lon), (data) => {
      ...
      const ctx = document.getElementById('FutureWeatherSummary');
      if (!ctx) return;
      const chart = new Chart(ctx.getContext('2d'), {
        type: 'line',
        data: {
          labels: [...Array(24).keys()].map(i => `in ${i}h`),
          datasets: [{
            label: 'Temperature (°C)',
            data: forecastList.slice(0, 24).map(item => item?.main?.temp),
